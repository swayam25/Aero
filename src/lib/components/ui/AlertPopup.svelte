            ...
            {#snippet child({ props, open })}
                {#if open}
                    <div {...props} transition:fade={{ duration: 100 }}>
                        <AlertDialog.Content class="flex max-w-[90%] md:max-w-[50%] flex-col gap-5 rounded-lg border border-slate-700 bg-slate-900 p-5 text-sm">
                            <div class="flex flex-col gap-2 text-left">
                                ...
                                <div
                                    class="flex flex-col items-center justify-center gap-2 md:gap-5 *:w-full *:cursor-pointer *:rounded-lg *:bg-slate-800 *:px-3 *:py-2 *:transition-all *:duration-200 *:not-first:flex *:not-first:w-full *:not-first:items-center *:not-first:justify-center *:not-first:gap-2 *:not-disabled:hover:bg-slate-800/50 *:disabled:brightness-80 md:flex-row md:justify-between"
                                >
